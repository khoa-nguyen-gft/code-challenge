...
public class BeanManager {
	...
	public <T> T getInstance(Class<T> clss) {
		try {
			T newInstance = clss.getConstructor().newInstance();
			
			for (Field field : clss.getDeclaredFields()) {
				Inject inject = field.getAnnotation(Inject.class);
				if (inject != null) {
					Supplier<?> supplier = register.get(field.getType());
					Object objectToInjection = supplier.get();
					field.setAccessible(true);
					field.set(newInstance, objectToInjection);
				}
			}
